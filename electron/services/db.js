...
if (activityCount.count === 0) {
    const insertActivity = db.prepare("INSERT INTO activities (name, color, created_at) VALUES (?, ?, ?)");
    const now = Date.now();
...
if (presetCount.count === 0) {
    const insertPreset = db.prepare("INSERT INTO timer_presets (minutes, created_at) VALUES (?, ?)");
    const now = Date.now();
