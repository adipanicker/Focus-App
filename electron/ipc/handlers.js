...
export function registerIpcHandlers() {
    ...
    ipcMain.on(IPC.TIMER_START, (_event, payload) => {
        timerService.start(payload.durationSeconds, payload.activityId, payload.type ?? "focus");
    });
    ...
    ipcMain.handle(IPC.ACTIVITIES_ADD, (_event, payload) => {
        const result = db
            .prepare("INSERT INTO activities (name, color, created_at) VALUES (?, ?, ?)")
            .run(payload.name, payload.color, Date.now());
        return db
            .prepare("SELECT * FROM activities WHERE id = ?")
            .get(result.lastInsertRowid);
    });
    ipcMain.handle(IPC.ACTIVITIES_UPDATE, (_event, payload) => {
        db.prepare("UPDATE activities SET name = ?, color = ? WHERE id = ?").run(payload.name, payload.color, payload.id);
        return db
            .prepare("SELECT * FROM activities WHERE id = ?")
            .get(payload.id);
    });
    ipcMain.handle(IPC.ACTIVITIES_DELETE, (_event, id) => {
    ...
    ipcMain.handle(IPC.PRESETS_ADD, (_event, payload) => {
        const result = db
    ...
    ipcMain.handle(IPC.PRESETS_UPDATE, (_event, payload) => {
        db.prepare("UPDATE timer_presets SET minutes = ? WHERE id = ?").run(payload.minutes, payload.id);
        return db
            .prepare("SELECT * FROM timer_presets WHERE id = ?")
            .get(payload.id);
    });
    ipcMain.handle(IPC.PRESETS_DELETE, (_event, id) => {
    ...
    ipcMain.handle(IPC.SETTINGS_UPDATE, (_event, payload) => {
